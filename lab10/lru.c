...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    //
    // 1) The virtual page is already in a physical page --> nothing happens
    //
    // 2) The virtual page is not in a physical page,
    //    and there is free physical page --> place in free physical page
    //
    // 3) The virtual page is not in a physical page,
    //    and there is no free physical page --> override oldest physical page
    //
    ...
    int page_num = 0;
    for (int i = 0; i < n_physical_pages; i++) {
        if (ipt[page_num].last_access_time > 
                    ipt[i].last_access_time) {
            page_num = i;
        }
    }

    int evicted = ipt[page_num].virtual_page;
    ipt[page_num].virtual_page = virtual_page;
    ipt[page_num].last_access_time = access_time;

    printf("Time %d: virtual page %d  - virtual page %d evicted ", 
            access_time, virtual_page, evicted);
